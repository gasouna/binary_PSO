...
for execution in executions:
    ...
    c_pop = json.load(open('{}/{}/population_after_crossing.json'.format(results, execution)))
    ...
    for i in pop:
        ...
        for j in range(n_bits * 2):
            ...
            c_dic["x"][j] = functions.binToDec(c_pop[i][j][0:n_bits])
            c_dic["y"][j] = functions.binToDec(c_pop[i][j][n_bits:])

        tmp = pd.DataFrame(dic)
        tmp['iter'] = i
        c_tmp = pd.DataFrame(c_dic)
        c_tmp['iter'] = i

        df = pd.concat([df, tmp])
        c_df = pd.concat([c_df, c_tmp])
    ...
    def get_date(iteration, dataset):
        if dataset == 0:
            ...
            return c_df[c_df.iter == iteration]['x'], c_df[c_df.iter == iteration]['y']
